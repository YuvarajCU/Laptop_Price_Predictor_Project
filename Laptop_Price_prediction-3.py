...
sns.pairplot(df, x_vars=['Weight'], y_vars=['Price_euros'], kind='scatter',
             height=5, aspect=1.5)
plt.show()
...
def fetch_processor(x):
    ...
    if cpu_name == 'Intel Core i7' or \
        cpu_name == 'Intel Core i5' or \
...
sns.catplot(x='Company', y='Price_euros', kind='bar', data=df, 
            height=5, aspect=1.5
...
sns.catplot(x='TypeName', y='Price_euros', kind='bar', 
            data=df).set_xticklabels(rotation=90)
sns.catplot(x='OpSys', y='Price_euros', kind='bar', 
            data=df).set_xticklabels(rotation=90)
...
Numerical = [col for col in df.columns if 
             (df[col].dtypes == 'int64') or 
             (df[col].dtypes == 'float64')]
...
X = df[['Ram', 'Weight', 'Price_euros', 'Touchscreen', 
        'Ips', 'ppi', 'SSD', 'HDD']]
y = df['Price_euros']
X = sm.add_constant(X)
...
model_comparison={}
model_comparison['SLR Vs Company']=[r2_score(y_test, y_pred), 
...
model_comparison['SLR Vs Ram']=[r2_score(y_test, y_pred) 
                                ,mean_squared_error(y_test, y_pred), 
                                mean_absolute_error(y_test, y_pred), 
                                (accuracies.mean())]
...
model_comparison['SLR Vs ppi']=[r2_score(y_test, y_pred), 
                                mean_squared_error(y_test, y_pred), 
                                mean_absolute_error(y_test, y_pred), 
                                (accuracies.mean())]
...
model_comparison['SLR Vs Weight']=[r2_score(y_test, y_pred), 
                                   mean_squared_error(y_test, y_pred), 
                                   mean_absolute_error(y_test, y_pred), 
                                   (accuracies.mean())]
...
model_comparison['SLR Vs HDD']=[r2_score(y_test, y_pred), 
                                mean_squared_error(y_test, y_pred), 
                                mean_absolute_error(y_test, y_pred), 
                                (accuracies.mean())]
...
ct = ColumnTransformer(
    transformers=[
        ('onehot', OneHotEncoder(sparse=False, 
                      handle_unknown='ignore'), [0, 2, 7, 10])
    ], remainder='passthrough')
...
ct = ColumnTransformer(
    transformers=[
        ('onehot', OneHotEncoder(sparse=False, 
                    handle_unknown='ignore'), [0, 1, 3, 8, 10])
    ], remainder='passthrough')
